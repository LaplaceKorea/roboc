...
void TrajectoryViewer::display(const std::vector<Eigen::VectorXd>& q_traj, 
  ...
  const bool success = viewer.initViewer("roboc::TrajectoryViewer");
  if(!success) {
    std::cout << "Failed in connecting to CORBA!!" << std::endl;
  }

  auto gui = gepetto::viewer::corba::gui();
  const auto window_id = gui->createWindow("roboc::TrajectoryViewer");
  ...
  gui->setBackgroundColor1("roboc::TrajectoryViewer", gepetto::viewer::corba::white);
  gui->setBackgroundColor2("roboc::TrajectoryViewer", gepetto::viewer::corba::white);
...
void TrajectoryViewer::printCurrentCameraTransform() const {
  auto gui = gepetto::viewer::corba::gui();
  const auto window_id = gui->getWindowID("roboc::TrajectoryViewer");
