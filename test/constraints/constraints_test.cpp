...
#include "roboc/constraints/constraints_data.hpp"
...
#include "roboc/constraints/joint_acceleration_upper_limit.hpp"
...
std::shared_ptr<Constraints> ConstraintsTest::createConstraints(Robot& robot) const {
  ...
  auto joint_accel_lower = std::make_shared<roboc::JointAccelerationLowerLimit>(robot, Eigen::VectorXd::Constant(robot.dimv(), -10));
  auto joint_accel_upper = std::make_shared<roboc::JointAccelerationUpperLimit>(robot, Eigen::VectorXd::Constant(robot.dimv(), 10));
  auto friction_cone = std::make_shared<roboc::FrictionCone>(robot, 0.7);
